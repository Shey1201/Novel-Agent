...
def _json_serializer(obj):
    """JSON 序列化辅助函数，处理 datetime 等特殊类型"""
...
def save_memory(memory: StoryMemory):
    ...
    components["world"].write_text(
        json.dumps(memory.bible.model_dump(), ensure_ascii=False, indent=2, default=_json_serializer), 
        encoding="utf-8"
    )
    components["characters"].write_text(
        json.dumps([c.model_dump() for c in memory.characters], ensure_ascii=False, indent=2, default=_json_serializer), 
        encoding="utf-8"
    )
    components["timeline"].write_text(
        json.dumps([t.model_dump() for t in memory.timeline], ensure_ascii=False, indent=2, default=_json_serializer), 
        encoding="utf-8"
    )
