...
DECLARE
    ...
    v_count INTEGER;
    v_name TEXT;
    ANON_USER_ID CONSTANT TEXT := 'a1b2c3d4-e5f6-7890-abcd-ef1234567890';
BEGIN
    RAISE NOTICE '🧪 开始数据库迁移测试...';
    ...
    RAISE NOTICE 'agents 表: %', CASE WHEN v_count > 0 THEN '✅ 存在' ELSE '❌ 不存在' END;
    
    SELECT COUNT(*) INTO v_count FROM information_schema.tables 
    WHERE table_schema = 'public' AND table_name = 'assets';
    RAISE NOTICE 'assets 表: %', CASE WHEN v_count > 0 THEN '✅ 存在' ELSE '❌ 不存在' END;
    
    SELECT COUNT(*) INTO v_count FROM information_schema.tables 
    WHERE table_schema = 'public' AND table_name = 'settings';
    RAISE NOTICE 'settings 表: %', CASE WHEN v_count > 0 THEN '✅ 存在' ELSE '❌ 不存在' END;
    
    SELECT COUNT(*) INTO v_count FROM information_schema.tables 
    WHERE table_schema = 'public' AND table_name = 'messages';
    RAISE NOTICE 'messages 表: %', CASE WHEN v_count > 0 THEN '✅ 存在' ELSE '❌ 不存在' END;
